...
class QA(MagicWord):
    ...
    execLocation = MagicWordConfig.EXEC_LOC_SERVER
    arguments = [("command", str, True), ("extra", int, True), ("extra2", int, True)]
    ...
    def handleWord(self, invoker, avId, toon, *args):
        ...
        playGrounds = ['ttc', 'dd', 'dg', 'mml', 'tb', 'ddl', 'lbhq', 'bbhq']
        
        playGround2Tier = {'ttc': Quests.TT_TIER,
                           'dd': Quests.DD_TIER,
                           'dg': Quests.DG_TIER,
                           'mml': Quests.MM_TIER,
                           'tb': Quests.BR_TIER,
        ...
        if command in playGrounds:
            for id in toon.getQuests():
                toon.removeQuest(id)
            toon.b_setRewardHistory(playGround2Tier[command], toon.getRewardHistory()[1])
        elif command == 'suitLevel':
            extra1 = args[1]
            toon.b_setCogLevels([extra1] * 4 + [0])
        elif command == 'suitType':
            extra1 = args[1]
            toon.b_setCogTypes([extra1] * 4 + [0])
        elif command == 'gagType':
            extra1 = args[1]
            extra2 = args[2]
            
            experience = Experience.Experience(toon.getExperience(), toon)
            
            experience.experience[extra1] = (ToontownBattleGlobals.Levels[extra1][extra2])
